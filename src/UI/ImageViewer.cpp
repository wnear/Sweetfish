...
ImageViewer::ImageViewer(TootData *tdata, unsigned int index, QWidget *parent, Qt::WindowFlags f)
    : QWidget(parent, f), now_index(index), first(true) {
    if (tdata == nullptr) return;

    for (unsigned int cnt = 0, size = tdata->getMediaData().size(); cnt < size; cnt++) {
        url_list.append(tdata->getMediaData().getEntry(cnt).getUrl());
    }
    init();
}

ImageViewer::ImageViewer(QStringList _url_list, unsigned int index, QWidget *parent, Qt::WindowFlags f)
...
void ImageViewer::init() {
    ...
    if (link_size <= now_index) now_index = 0;
...
void ImageViewer::setImage(const QString &url) {
    ...
    connect(rep, &QNetworkReply::finished, [rep]() {
        if (rep->attribute(QNetworkRequest::SourceIsFromCacheAttribute).toBool()) qDebug() << "Reply from cache" << __LINE__;
    });
...
void ImageViewer::nextImage() {
    ...
    if (now_index == url_list.size() - 1) next_button->setEnabled(false);
}
...
void ImageViewer::save() {
    ...
    QString tempname = media_link.split("/").constLast();  // http://doc.qt.io/qt-5/qstring.html#split に「If sep
    ...
    for (int cnt = spfilters.size() - 1; cnt >= 0; filter.append(" *." + spfilters.at(cnt)), cnt--)
    ...
    if (filename.isEmpty() || !iml->pixmap()->save(filename))
        QMessageBox::critical(this, tr("画像の詳細 ") + APP_NAME, tr("画像の保存に失敗しました。"));
}
