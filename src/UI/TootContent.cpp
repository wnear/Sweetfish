...
void TootContent::createActions() {
    if (mode & Mode::Info) {
        popup
            ->addAction(this->style()->standardIcon(QStyle::SP_TitleBarCloseButton) /*少し意図がずれてる気が*/, tr("Delete(&D)"),
                        this, &TootContent::triggeredAction)
            ->setData('d');
        return;  //最低限の表示のみ
    }
    //リトゥート
    ...
    if (const TootUrlData url_list = tdata->getUrlData(); url_list.count() != 0) {
        ...
        for (unsigned int cnt = 0, size = url_list.count(); cnt < size; cnt++) {
            QMenu *url_menu =
                new QMenu((url_list.getDisplayUrl(cnt).size() > 15) ? url_list.getDisplayUrl(cnt).left(15).append("...")
...
void TootContent::drawToot() {
    ...
    if (!icon->setPixmapByName(tdata->getOriginalAccountData().getAvatar())) {  //アイコンのキャッシュがない
        connect(net.get(tdata->getOriginalAccountData().getAvatar()), &QNetworkReply::finished, icon,
    ...
    if (tdata->getMediaData().size() && !(mode & Mode::Info)) {
        ...
        if (media_data.getEntry(0).getType() == "video" || media_data.getEntry(0).getType() == "gifv") {  //動画(一つのみ対応)
            ...
            if (!iml->setPixmapByName(video_entry.getPreviewUrl())) {  //キャッシュなし
                ...
                /*else*/ connect(net.get(video_entry.getPreviewUrl()), &QNetworkReply::finished, iml,
                                 &ImageLabel::setPixmapByNetwork);
            }
...
void TootContent::drawQuoteToot(QString full_url, QVBoxLayout *text_box) {
    ...
    connect(net.get(MastodonUrl::scheme + url[1] + MastodonUrl::statuses + "/" + id), &QNetworkReply::finished, this,
            [this, quote] {
