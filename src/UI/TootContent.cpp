...
TootContent::TootContent(TootData *init_tdata, Mode init_mode, MainWindow *rw /*ImageViewerなどに渡す*/, QWidget *parent,
                         Qt::WindowFlags f)
...
void TootContent::createActions() {
  if (mode & Mode::Info) {
    popup
        ->addAction(this->style()->standardIcon(QStyle::SP_TitleBarCloseButton) /*少し意図がずれてる気が*/, tr("Delete(&D)"), this,
                    &TootContent::triggeredAction)
        ->setData('d');
    return;  //最低限の表示のみ
  }
  //リトゥート
  popup->addSection((tdata->getContent().size() >= 16) ? tdata->getContent().left(16 - 3).append("...")
                                                       : tdata->getContent());  //使い方合ってるんかいな...
  popup->addAction(QIcon(":/bst.png"), tr("Boost(&B)"), this, &TootContent::triggeredAction)->setData('b');

  //引用トゥート
  popup->addAction(QIcon(":/bst.png"), tr("Quote Toot(&Q)"), this, &TootContent::triggeredAction)->setData('q');

  //リプライ
  popup->addAction(QIcon(":/rp.png"), tr("Reply(&R)"), this, &TootContent::triggeredAction)->setData('r');

  //お気に入り(今はいいねだが...)
  popup->addAction(QIcon(":/fav.png"), tr("Favourite(&F)"), this, &TootContent::triggeredAction)->setData('f');

  //削除(権限がある場合)
  if (tdata->isTootOwner())
    popup
        ->addAction(style()->standardIcon(QStyle::SP_TitleBarCloseButton) /*少し意図がずれてる気が*/, tr("Delete(&D)"), this,
                    &TootContent::triggeredAction)
        ->setData('d');
  //新しいウィンドウで開く(取っておきたいなど)
  popup->addAction(style()->standardIcon(QStyle::SP_TitleBarMaxButton), tr("Open in new window(&W)"), this,
  ...
  if (const TootUrlData url_list = tdata->getUrlData(); url_list.count() != 0) {
    ...
    for (unsigned int cnt = 0, size = url_list.count(); cnt < size; cnt++) {
      QMenu *url_menu = new QMenu((url_list.getDisplayUrl(cnt).size() > 15) ? url_list.getDisplayUrl(cnt).left(15).append("...")
                                                                            : url_list.getDisplayUrl(cnt),
                                  popup);

      QAction *browser_open_action = new QAction(tr("ブラウザで開く"));
      browser_open_action->setData(cnt);
      connect(browser_open_action, &QAction::triggered, this, &TootContent::openUrl);
      url_menu->addAction(browser_open_action);
      QAction *url_copy_action = new QAction(tr("コピー"), url_menu);
      connect(url_copy_action, &QAction::triggered, this, &TootContent::copyUrl);
  ...
  popup->addAction(tdata->getOriginalAccountData().getDisplayName(), this, [this] {
    UserInfoBox *box = new UserInfoBox(tdata->getOriginalAccountData(), root_window, Qt::Window);
    box->show();
  });
  if (!tdata->getApplicationName().isEmpty()) {
    popup->addSection(tr("クライアント"));
    popup
        ->addAction(tdata->getApplicationName(), this,
                    [this] { QDesktopServices::openUrl(QUrl(((qobject_cast<QAction *>(sender()))->data().toString()))); })
 ...
 */
void TootContent::triggeredAction() { return emit action(tdata, (qobject_cast<QAction *>(sender()))->data().toChar().toLatin1()); }
...
void TootContent::openUrl() {
  QString &&url = tdata->getUrlData().getFullUrl(qobject_cast<QAction *>(sender())->data().toInt());
...
void TootContent::drawToot() {
  ...
  if (!icon->setPixmapByName(tdata->getOriginalAccountData().getAvatar())) {  //アイコンのキャッシュがない
    connect(net.get(tdata->getOriginalAccountData().getAvatar()), &QNetworkReply::finished, icon, &ImageLabel::setPixmapByNetwork);
  }
  main_box->addWidget(icon, 0, Qt::AlignTop);

  if (tdata->getBoostedData() != nullptr) {  // Boost
    QLabel *boosted_user_name = new QLabel(((tdata->getAccountData().getDisplayName().size() >= 20)
                                                ? tdata->getAccountData().getDisplayName().left(20 - 3).append("...")
                                                : tdata->getAccountData().getDisplayName()) +
                                           tr(" boosted"));
    boosted_user_name->setStyleSheet("font-size:10px;color:lime;");  // small指定ができない
    boosted_user_name->setWordWrap(true);
    text_box->addWidget(boosted_user_name);
  }

  QLabel *display_name = new QLabel((tdata->getOriginalAccountData().getDisplayName().size() >= 20)
                                        ? tdata->getOriginalAccountData().getDisplayName().left(20 - 3).append("...")
  ...
  QLabel *user_name = new QLabel('@' + tdata->getOriginalAccountData().getAcct());
  ...
  if (tdata->getMediaData().size() && !(mode & Mode::Info)) {
    ...
    if (media_data.getEntry(0).getType() == "video" || media_data.getEntry(0).getType() == "gifv") {  //動画(一つのみ対応)
      ...
      if (!iml->setPixmapByName(video_entry.getPreviewUrl())) {  //キャッシュなし
        ...
        /*else*/ connect(net.get(video_entry.getPreviewUrl()), &QNetworkReply::finished, iml, &ImageLabel::setPixmapByNetwork);
      }
      ...
      for (unsigned int cnt = 0, size = media_data.size(); cnt < size; cnt++) {
        ...
        if (!iml->setPixmapByName(image_entry.getUrl())) {  //キャッシュなし
          /* iml->setPixmap(style()->standardIcon(QStyle::SP_MessageBoxWarning).pixmap(100,
           * 45));*/
          connect(net.get(image_entry.getUrl()), &QNetworkReply::finished, iml, &ImageLabel::setPixmapByNetwork);
        }
        connect(iml, &ImageLabel::clicked, this, &TootContent::showPicture);
  ...
  for (unsigned int cnt = 0, size = tdata->getUrlData().size(); cnt < size; cnt++) {
  ...
  //その他情報
  auto time = QLocale::system().toString(tdata->getDateTime(), QLocale::ShortFormat);
  QLabel *info_text = new QLabel(time);
...
void TootContent::drawQuoteToot(QString full_url, QVBoxLayout *text_box) {
  ...
  if (!QRegExp("^\\d+$").exactMatch(id)) return;
  ...
  connect(net.get(MastodonUrl::scheme + url[1] + MastodonUrl::statuses + "/" + id), &QNetworkReply::finished, this, [this, quote] {
    QNetworkReply *rep = qobject_cast<QNetworkReply *>(sender());
    quote->setTootData(new TootData(QJsonDocument::fromJson(rep->readAll()).object()), true);
    rep->deleteLater();
  });
}
...
void TootContent::drawCard(QVBoxLayout *text_box) {
  ...
  QLabel *title_label =
      new QLabel((card_data.getTitle().size() >= 16) ? card_data.getTitle().left(16 - 3).append("...") : card_data.getTitle());
  ...
  if (!card_data.getPreviewUrl().isEmpty()) {
    ...
    if (!preview_icon->setPixmapByName(card_data.getPreviewUrl())) {  //アイコンのキャッシュがない
      connect(net.get(card_data.getPreviewUrl()), &QNetworkReply::finished, preview_icon, &ImageLabel::setPixmapByNetwork);
    }
    connect(preview_icon, &ImageLabel::clicked, this, &TootContent::showCardPicture);
    card_box->addWidget(preview_icon);

    if (!card_data.getAuthorName().isEmpty()) {
      QLabel *author_label =
          new QLabel(tr("著者:") + "<a href=\"" + card_data.getAuthorUrl() + "\">" +
                     ((card_data.getAuthorName().size() >= 16) ? card_data.getAuthorName().left(16 - 3).append("...")
    ...
    if (!card_data.getProviderName().isEmpty()) {
      QLabel *provider_label =
          new QLabel(tr("提供者:") + "<a href=\"" + card_data.getProviderUrl() + "\">" +
                     ((card_data.getProviderName().size() >= 16) ? card_data.getProviderName().left(16 - 3).append("...")
...
void TootContent::showPicture(unsigned int index) {
  ...
  if (tdata->getMediaData().getEntry(0).getType() == "video") {  //動画
    ...
    ImageViewer *viewer = new ImageViewer(tdata, index, root_window, Qt::Window);
    viewer->show();
  }
}
...
void TootContent::showCardPicture(unsigned int index) {
  ...
  // return;
  qDebug() << __func__ << __LINE__;
  ImageViewer *viewer = new ImageViewer(QStringList(tdata->getCardData().getPreviewUrl()), index, root_window, Qt::Window);
  viewer->show();
}
...
void TootContent::openWindow() {
  if (tdata == nullptr) return;
  TootContent *window = new TootContent(new TootData(*tdata), mode, root_window, root_window, Qt::Window);
 ...
 */
void TootContent::transferAction(TootData *ori, unsigned char act) { return emit action(ori, act); }
