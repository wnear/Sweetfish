...
VideoPlayer::VideoPlayer(TootData *tdata, QWidget *parent, Qt::WindowFlags f) : QWidget(parent, f) {
    if (tdata == nullptr || !tdata->getMediaData().size()) return;
    media_data = tdata->getMediaData();
    QString media_url =
        media_data.getEntry(0).getRemoteUrl().isEmpty() ? media_data.getEntry(0).getUrl() : media_data.getEntry(0).getRemoteUrl();
    ...
    connect(video_player, &Phonon::VideoPlayer::finished, video_player,
            static_cast<void (Phonon::VideoPlayer::*)(void)>(&Phonon::VideoPlayer::play));  // Loop
...
void VideoPlayer::createButtons(QVBoxLayout *main_layout) {
    ...
    connect(start, &QPushButton::clicked, video_player,
            static_cast<void (Phonon::VideoPlayer::*)(void)>(&Phonon::VideoPlayer::play));
