...
void MainWindow::setUpMenu() {
    ...
    QAction *always_top_action =
        window_menu->addAction(style()->standardIcon(QStyle::SP_ArrowUp /*QStyle::SP_TitleBarShadeButton*/),
...
void MainWindow::showNotification(TootNotificationData *nfdata) {
    ...
    switch (nfdata->getType()) {
        ...
        case TootNotificationData::Event::Mention:
            message = nfdata->getAccount().getDisplayName() + tr("さんがあなたに向けてトゥートしました。\n") +
    ...
    try {
        if (unsigned int c = toot_info->getNumOfImage()) {
            ...
            connect(upload, &MediaUpload::aborted, this, [this] {
                ...
                if (QMessageBox::question(this, APP_NAME, tr("アップロード中にエラーが発生しました。再試行しますか。")) ==
                    QMessageBox::Yes)
                    upload->retry();
                else
                    delete upload;
        ...
        connect(
            mstdn->requestToot(text, QByteArray(), (toot_info->getReplyToot()) ? toot_info->getReplyToot()->getId() : QByteArray()),
